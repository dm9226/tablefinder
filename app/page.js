...
function TableFinder() {
  ...
  var detectLocation = function() {
    ...
    navigator.geolocation.getCurrentPosition(
      async function(pos) {
        ...
        try {
          ...
          setUserLocation({
            ...
            zip: data.postcode || "",
            neighborhood: (data.localityInfo && data.localityInfo.administrative && (data.localityInfo.administrative[3] && data.localityInfo.administrative[3].name || data.localityInfo.administrative[2] && data.localityInfo.administrative[2].name)) || "",
      ...
      function() { setLocationError(true); setLocationLoading(false); },
      { timeout: 10000 }
    );
  };
  ...
  var search = async function(overrideText) {
    ...
    try {
      var now = new Date();
      var localDate = now.toLocaleDateString("en-CA");
      var localHour = now.getHours();
      var localTime = localHour.toString().padStart(2, "0") + ":" + now.getMinutes().toString().padStart(2, "0");
      ...
      setSearchHistory(function(prev) { return [text].concat(prev.filter(function(h) { return h !== text; })).slice(0, 5); });
      setTimeout(function() { if (resultsRef.current) resultsRef.current.scrollIntoView({ behavior: "smooth", block: "start" }); }, 100);
    } catch (e) {
  ...
  var locationLabel = userLocation ? (userLocation.neighborhood || userLocation.city) : null;
  var locationDetail = locationLabel ? locationLabel + (userLocation.zip ? " " + userLocation.zip : "") : null;
    ...
    <div style={{ minHeight: "100vh", background: "#1A1612", color: "#F0E6D8", fontFamily: "'Outfit', sans-serif" }}>
      ...
      <header style={{ position: "sticky", top: 0, background: "rgba(26,22,18,0.95)", backdropFilter: "blur(12px)", zIndex: 10, borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
        <div style={{ maxWidth: 780, margin: "0 auto", padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={function() { setResults(null); setInput(""); }}>
        ...
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          <div style={{ display: "flex", gap: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: "6px 6px 6px 18px", alignItems: "center" }}>
            <input
              ...
              placeholder={userLocation && userLocation.city ? "Italian for 2, tonight in " + userLocation.city + "..." : "Italian for 2, tonight..."}
              disabled={loading}
        ...
        <div style={{ maxWidth: 780, margin: "0 auto", padding: "20px 20px", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, color: "#8A7E70", fontSize: 14 }}>
          <div style={{ display: "flex", gap: 4 }}>
            {[0, 1, 2].map(function(i) {
              return <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#E8A86D", animation: "pulse 1.2s infinite " + (i * 0.2) + "s" }} />;
            })}
          </div>
      ...
      {results && (
        <div ref={resultsRef} style={{ maxWidth: 780, margin: "0 auto", padding: "0 20px 60px" }}>
          ...
          {results.structured && results.structured.restaurants && results.structured.restaurants.length > 0 ? (
            <div>
              <SearchSummary structured={results.structured} />
              {results.structured.restaurants.map(function(r, i) {
                return <RestaurantCard key={i} restaurant={r} />;
              })}
            </div>
          ) : (
      ...
      <footer style={{ textAlign: "center", padding: "20px 24px", fontSize: 11, color: "rgba(255,255,255,0.15)" }}>
        {"Real-time availability from Resy, OpenTable & Yelp"}
      </footer>
    </div>
  );
}
